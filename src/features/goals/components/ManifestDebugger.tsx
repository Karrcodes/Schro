...
export default function ManifestDebugger() {
    const { debugLogs, addDebugLog } = useGoals()
    ...
    const handleTestGenerate = async () => {
        ...
        try {
            ...
            // Using the same API endpoint but with custom title as prompt and type=goal
            const res = await fetch(`/api/studio/cover?title=${encodeURIComponent(prompt)}&type=goal&json=true&t=${timestamp}`)
        ...
        <div className="mt-20 border-t border-black/5 pt-12 pb-24">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[11px] font-black uppercase tracking-widest text-black/40">Manifest Engine Debugger</h3>
                </div>
            </div>
