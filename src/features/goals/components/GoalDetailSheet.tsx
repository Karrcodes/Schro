...
import { X, Calendar, Clock, Trash2, CheckCircle2, Circle, Pencil, Loader2, Wand2, PiggyBank } from 'lucide-react'
...
interface GoalDetailSheetProps {
    ...
    onDeleteGoal: (id: string, cascade?: boolean) => Promise<void>
...
export default function GoalDetailSheet({ goal, isOpen, onClose, onToggleMilestone, onUpdateMilestone, onDeleteGoal, onEdit }: GoalDetailSheetProps) {
    ...
    const activeGoal = goal || displayGoal

    if (!activeGoal) return null

    const isGenerating = activeGoal ? generatingGoalIds.includes(activeGoal.id) : false
    const totalMilestones = activeGoal.milestones?.length || 0
    const completedMilestones = activeGoal.milestones?.filter(m => m.is_completed).length || 0
            ...
            {isOpen && activeGoal && (
                <>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <span className="px-2 md:px-2.5 py-1 bg-black/[0.04] text-black/50 rounded-lg text-[9px] md:text-[10px] font-bold uppercase tracking-wider border border-black/5">
                                            {activeGoal.timeframe} Horizon
                                        </span>
                                    </div>
                                    <h2 className="text-[24px] md:text-[32px] font-bold text-black tracking-tight leading-[1.1]">{activeGoal.title}</h2>
                                    <p className="text-[14px] md:text-[15px] text-black/50 font-medium leading-relaxed max-w-xl">{activeGoal.description || 'Define your strategic path.'}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                ...
                                <div className="md:col-span-1 space-y-6">
                                    {activeGoal.vision_image_url ? (
                                        ...
                                        <div
                                            onClick={() => !isGenerating && regenerateGoalCover(activeGoal.id)}
                                            className={cn(
                                    ...
                                    <div className="p-6 bg-black/[0.02] rounded-2xl border border-black/5 space-y-4">
                                        ...
                                        <div className="flex items-center gap-4 pt-2">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[15px] font-bold uppercase tracking-tight text-emerald-600">{activeGoal.status}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="md:col-span-2 space-y-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="p-5 bg-[#fafafa] rounded-2xl border border-black/[0.06] space-y-1 shadow-sm">
                                                ...
                                                <p className="text-[14px] font-bold text-black">{activeGoal.target_date ? new Date(activeGoal.target_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Set Deadline'}</p>
                                            </div>
                                            <div className="p-5 bg-[#fafafa] rounded-2xl border border-black/[0.06] space-y-1 shadow-sm">
                                                <div className="flex items-center gap-2 text-black/30 mb-2">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className={cn(
                                                        ...
                                                        activeGoal.priority === 'super' ? "bg-purple-600 animate-pulse shadow-[0_0_8px_rgba(147,51,234,0.5)]" :
                                                            activeGoal.priority === 'high' ? "bg-red-500" :
                                                                activeGoal.priority === 'mid' ? "bg-amber-500" :
                                                                    "bg-black/20"
                                                    )} />
                                                    <p className={cn(
                                                        "text-[14px] font-bold uppercase tracking-tight",
                                                        activeGoal.priority === 'super' ? "text-purple-600" :
                                                            activeGoal.priority === 'high' ? "text-red-600" :
                                                                activeGoal.priority === 'mid' ? "text-amber-600" :
                                                                    "text-black"
                                                    )}>
                                                        {activeGoal.priority}
                                                    </p>
                                    ...
                                    {/* Linked Savings */}
                                    {activeGoal.linked_savings_id && (() => {
                                        const savings = activeGoal.linked_savings_type === 'manual'
                                            ? financeGoals.find(f => f.id === activeGoal.linked_savings_id)
                                            : pots.find(p => p.id === activeGoal.linked_savings_id)
                                        ...
                                        return (
                                            <div className="pt-6 border-t border-black/5 space-y-4">
                                                ...
                                                <div className="bg-emerald-50/30 border border-emerald-500/10 rounded-2xl p-5">
                                                    ...
                                                    <div className="text-[10px] font-bold text-emerald-600/60 uppercase tracking-widest mt-4 flex items-center gap-1.5">
                    ...
                    <ConfirmationModal
                        ...
                        title="Delete Goal?"
                        message={`Are you sure you want to permanently delete "${activeGoal.title}"? All associated milestones and visual manifestations will be removed.`}
                        confirmText="Delete"
                        type="danger"
                    />
                </>
